...
double NPV=100.0, N=3.0*NPV;/*El # de particulas virtuales*/
using namespace std;
...
//	return x*x*x-x*x+2;
	return (1.0/a)*(x)-(1.0/(2.0*PI))*sin(2.0*PI*x/a)-1.0/NPV;
}
...
int main(){
	...
	for(int i=0; i<(int)NPV; ++i){
		...
		if(i==0){
			...
			double e= (funct(r)-funct(p[i-1].r)-1.0/(NPV))/(dfunct(r));
			while(abs(e)>=EPSILON){
				e= (funct(r)-funct(p[i-1].r)-1.0/(NPV))/(dfunct(r));
				r=r-e;
			}	
			p[i].r=r;
		}
...
//	ofstream file("rN500h25.dat");
	...
	for(int i=0; i<(int)N; ++i){
		...
		if(i==0){
			p[i].r=p[i].r/2.0-a;  /*Para la primera posición*/
//			file << i << " " << p[i].r << '\n';
		}else{				/*Para las demás posiciones*/
			p[i].r=(p[i].r+b[i-1])/2.0-a; /*surge de tomar en cuenta (b[i].r-b[i-1].r)/2+b[i-1].r*/
//			file << i << " " << p[i].r << '\n';
		}
		...
		if(i >= (int)NPV  && i<(int)(2.0*NPV)){
			p[i].r=p[i-(int)NPV].r+a;
		}
		
		if(i >= (int)(2.0*NPV) && i<(int)(3.0*NPV)){
			p[i].r=p[i-(int)(2.0*NPV)].r+2.0*a;
		}
		
	}
//	file.close();
	double  h[(int)N] /*200/N*/, alpha[(int)N] /*(1.0/(h*sqrt(PI)))*/, beta[(int)N]/*(2.0/h)*/, delta[(int)N] /*2.0/(h*h)*/,  n, m=1.0/NPV; /*h[N] es el valor de la distancia de suavizado para cada partícula, alpha, beta y delta solo son valores que ayudan a reducir la expresión*/
...
//	ofstream file5("PPEN100h45.dat");
//	for(int i=0; i<(int)N; ++i){
//		file5 << p[i].r << " " << p[i].in.size() << '\n'; 
//	}
//	file5.close();
	/*Se calcula el valor de la densidad*/
	ofstream file2("DPN100h15.dat");
		for(int i=0; i<(int)N; ++i){
			p[i].Rho=0;
		...
		file2.close();
	/*Se cálcula el error*/
//		ofstream file3("errorN100h25.dat");
//		for(int i=0; i<(int)N;++i){
//			double s=sin(PI*p[i].r*0.2);
//			double den=0.4*s*s;
